    ...
    public class RadarChartGraphic : Graphic {
        ...
        private void GenerateBroadLine(VertexHelper vh,float k) {
            ...
            for(var i = 0;i < _valueCount;i++) {
                ...
                vh.AddVert(GenerateUIVertex(GetPos(i) * (_baseRadius * k * _valueList[i] - dif),_lineColor)); //Inner Vertex
            }
            for(var i = 0;i < _valueCount;i++) {
                vh.AddTriangle(baseCount + 2 * i,baseCount + (2 * i + 2) % (_valueCount * 2),baseCount + (2 * i + 3) % (_valueCount * 2));
